...
globals
  [ %infected            ;; what % of the population is infectious
    ...
    lifespan-age         ;; the lifespan of a turtle in years
    chance-reproduce     ;; the probability of a turtle generating an offspring each tick
    carrying-capacity    ;; the number of turtles that can be in the world at one time
    population           ;; the number of turtles in the population
...
to setup-turtles
  create-turtles number-people
    [ setxy random-xcor random-ycor
      ...
      set population number-people
      set turtle-shape "face sad" ;; the turtle shape will be a sad face to indicate that no masks are worn
      get-healthy ]
  ask n-of 10 turtles
    [ get-sick ]
  if wear-mask  ;; new if statement for the toggle switch that controls whether the turtles are wearing masks
    [ set infectiousness infectiousness * 0.8  ;; reduces the infectiousness of the disease when wear-mask is turned on
      set turtle-shape "face happy"]  ;; changes the turtle shape to a happy face to when wear-mask is turned on
end
...
to setup-constants
  set lifespan-age lifespan * 52      ;; changes lifespan from a hardcoded vairable to a variable that the user can change, which affects population size over time
  set carrying-capacity carrying-capacity-amount    ;; changes carrying-capacity from a hard-coded variable to a changeable one, which also affects population sizes
...
turtle-shape
turtle-shape
"person" "circle" "face happy" "face sad"
2
...
410
195
When the wear-mask switch is turned on and then the setup button is pressed, the infectiousness is lowered by 20%. Watch the infectiousness slider change when you press setup!
11
0.0
1
...
400
236
Lifespan affects the population count over time as disease spreads.
11
0.0
1
...
410
270
Wearing masks has been proven to lower the spread of disease. This variable will cause a visible change in infectiousness.
11
0.0
1
...
405
326
Turtle shape will change in accordance to whether wear-mask is off or on.
11
0.0
1
...
410
96
Must input number before starting simulation.
11
13.0
1
